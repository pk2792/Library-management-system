...
//			int branch_id = Integer.parseInt(request.getParameter("brid"));
			...
			List<BorrowMstBO> borrowlist = dao.getUserBorrowedBooks(reader_id);
			String bids[] = new String[borrowlist.size()];
			if(borrowlist.size() > 0){
				for(int i=0;i<borrowlist.size();i++){
					bids[i] = borrowlist.get(i).getBook_id()+"/"+borrowlist.get(i).getBranch_id()+"/"+borrowlist.get(i).getStatus();
				}
			}
			...
			if(searchResult.size() > 0){
				...
				writer.print("<br/><br/><div><input class='button button-3d button-primary button-rounded' type='button' value='Checkout' onclick='submitdata();'/></div>");
			}else{
