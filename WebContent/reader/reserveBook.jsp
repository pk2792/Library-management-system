...
function search(){
	$.ajax({
		url: "<%=request.getContextPath()%>/reserve?bid="+$("#bid").val()+"&aid="+$("#aid").val()+"&pid="+$("#pid").val()+"", 
...
function submitdata(){
	...
	var bc = document.getElementById("resrved").value;
	var c = 10 - parseInt(bc);
	if(a > 0){
		if(a <= c){
			if(confirm("Are you sure ?")){
				document.reserve.action = "<%=request.getContextPath()%>/reserveDone";
		...
		alert("You must select atleast one book to reserve !!!");
	}
}
