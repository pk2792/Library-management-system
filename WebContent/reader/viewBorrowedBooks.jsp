...
function submitdata(){
	...
	if(a > 0){
		if(confirm("Are you sure ?")){
			document.checkout.action = "<%=request.getContextPath()%>/ReturnBooks";
...
<!--           <h2 class="star"><span>Admin</span> Functionalities</h2> -->
      ...
      <div class="mainbar">
        <div class="article">
          ...
          <div align="center" style="color: red;font-weight: bold;">NOTE : You must return book within 20 days prior to its Borrow Date.After 20 days you will be fined 20 cents for each day.</div>
        <br/>
        ...
        <%for(SearchBO s : list){ %>
        	<tr>
        		...
        		<td><%=s.getBorrow_date() %></td>
        		<td><%if(s.getReturn_date() != null){ %><%=s.getReturn_date()%><%}else{ %>----<%} %></td>
        		<td><%if(s.getAuthorfname().equalsIgnoreCase("borrow")){ %><%=s.getDays() %>&nbsp;Day(s)<%}else{ %><b style="color: green;">Returned</b><%} %></td>
        		<td>
        		<%if(s.getAuthorfname().equalsIgnoreCase("borrow")){ %>
        ...
        <div align="center"><input class="button button-3d button-primary button-rounded" type='button' value='Return Book(s)' onclick='submitdata();'/></div>
        <%}else{ %>
        <br/><br/>
